        ...
        <ul class="flex items-center flex-shrink-0 space-x-6">
            ...
            <li class="flex">
                <button class="rounded-md focus:outline-none focus:shadow-outline-purple"
                    x-on:click="darkMode = ! darkMode" aria-label="Toggle color mode">
            ...
            <li class="relative" x-data="{ isProfileMenuOpen: false }">
                <button class="align-middle rounded-full focus:shadow-outline-purple focus:outline-none"
                    x-on:click="isProfileMenuOpen = ! isProfileMenuOpen" x-on:click.away="isProfileMenuOpen = false"
                    x-on:keydown.escape="isProfileMenuOpen = false" aria-label="Account" aria-haspopup="true">
                    <img class="object-cover w-8 h-8 rounded-full"
                        src="https://ui-avatars.com/api/?name={{ auth()->user()->name }}&color=9964ea&background=ebf4ff"
                    ...
                    <ul x-transition:leave="transition ease-in duration-150" x-transition:leave-start="opacity-100"
                        ...
                        <li class="flex">
                            <form class="w-full" method="POST" action="{{ route('logout') }}" x-data>
                                @csrf
                                <x-partials.dropdown-link href="{{ route('logout') }}"
